...
for sim_k = 1:nIter
    ...
    while (max(abs(dlambda)) > toll) && ADMM_iter < MaxIt
        ...
        if ADMM_iter == 1
            ...
            p_final1 = nlp_par1;
            [sol1p,elapsed] = SolveLinSysOnline(Primal1,Dual1,...
                p_init1,p_final1,par1);
            
            Primal1 = Primal1 + full(sol1p.dx);
            Dual1.lam_g = Dual1.lam_g + sol1p.lam_g;
            Dual1.lam_x = Dual1.lam_x + sol1p.lam_x;
            p_init1 = nlp_par1;
            w01 = Primal1;
        end
        ...
        if ADMM_iter ==1
            ...
            Primal2 = w02;
            Dual2.lam_g = full(sol2.lam_g);
            Dual2.lam_x = full(sol2.lam_x);
            p_init2 = nlp_par2;
            
        else
            p_final2 = nlp_par2;
            [sol2p,elapsed] = SolveLinSysOnline(Primal2,Dual2,...
                p_init2,p_final2,par2);
            
            Primal2 = Primal2 + full(sol2p.dx);
            Dual2.lam_g = Dual2.lam_g + sol2p.lam_g;
            Dual2.lam_x = Dual2.lam_x + sol2p.lam_x;
            p_init2 = nlp_par2;
            w02 = Primal2;
        end
